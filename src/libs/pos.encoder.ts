...
export class PosEncoder {
     ...
     * @param options  Object containing configuration options
     */
    private _reset({ codepage, ...options }: OptionsType = {}) {
        this._options = {
            width: null,
            embedded: false,
            wordWrap: true,
    ...
    private _encode(value: string) {
        if (this._codepage !== 'auto') {
            return cpEncoder.encode(value, this._codepage);
        }

        let codepages: Record<Partial<KeysDefinitions2>, number>;
     ...
     *
     * @param value  Add array of numbers, arrays, buffers or Uint8Arrays to add to the buffer
     */
    private _queue(value: (number | Uint8Array)[]) {
        for (const item of value) {
            this._queued.push(item);
        }
    }
        ...
        if (this._embedded) {
            ...
            if (this._state.align == 'center') {
                ...
                if (indent + remainder > 0) {
                    this._queued.unshift(
                        new Array(indent + remainder).fill(0x20),
                    );
                }
            }
     ...
     */
    private _wrap(value: string, position?: number): string[] {
        if (position || (this._options.wordWrap && this._options.width)) {
            ...
            const result = (w(indent + value) as string)
     ...
     */
    private _getCodepageIdentifier(codepage: string): number {
        let codepages: Record<Partial<KeysDefinitions2>, number>;
        if (typeof this._options.codepageMapping === 'string') {
            codepages = codepageMappings[this._options.codepageMapping];
        } else {
     ...
     */
    public codepage(codepage: KeysDefinitions2 | 'auto') {
        if (codepage === 'auto') {
            this._codepage = codepage;
            return this;
        }

        if (!cpEncoder.supports(codepage)) {
     ...
     */
    public text(value: string, wrap?: number) {
     ...
     * @param wrap  Wrap text after this many positions
     */
    public line(value: string, wrap?: number) {
     ...
     */
    public underline(value?: boolean | number) {
     ...
     *
     * @param width  The width of the text, 1 - 8
     */
     ...
     *
     * @param height  The height of the text, 1 - 8
     */
     ...
     */
    public size(value: 'small' | 'normal') {
     ...
     */
    public align(value: 'left' | 'center' | 'right') {
        const alignments = {
            left: 0x00,
            center: 0x01,
            right: 0x02,
     ...
     * @param data  Array containing rows. Each row is an array containing cells.
     ...
     */
    public table(columns: any[], data: any[]) {
        if (this._cursor !== 0) {
            this.newline();
        }
        ...
        for (let r = 0; r < data.length; r++) {
            ...
            for (let c = 0; c < columns.length; c++) {
                ...
                if (typeof data[r][c] === 'string') {
                    ...
                    for (let f = 0; f < fragments.length; f++) {
                        if (columns[c].align == 'right') {
                            cell[f] = this._encode(
                                fragments[f].padStart(columns[c].width),
                            );
                        } else {
                            cell[f] = this._encode(
                                fragments[f].padEnd(columns[c].width),
                            );
                        }
                    }
                }
                ...
                if (typeof data[r][c] === 'function') {
                    const columnEncoder = new PosEncoder({
                        ...this._options,
                ...
                if (lines[c].length < maxLines) {
                    for (let p = lines[c].length; p < maxLines; p++) {
                        ...
                        if (verticalAlign == 'bottom') {
                            lines[c].unshift(
                                new Array(columns[c].width).fill(0x20),
                            );
                        } else {
                            lines[c].push(
                                new Array(columns[c].width).fill(0x20),
    ...
    public rule(options: { style?: 'single' | 'double'; width?: number } = {}) {
        ...
        this._queue([
            ...
            ...new Array(options.width).fill(
                options.style === 'double' ? 0xcd : 0xc4,
            ),
        ]);
     ...
     * @param contents  A string value, or a callback function.
     *                 The first parameter of the callback is the encoder object on
     *                 which the function can call its methods.
     */
    public box(
        ...
        contents: string | ((encoder: PosEncoder) => void),
    ) {
        options = {
            style: 'single',
     ...
     */
    public barcode(value: string, symbology: string, height: number) {
        if (this._embedded) {
            throw new Error(
                'Barcodes are not supported in table cells or boxes',
            );
        }
        ...
        if (symbology in symbologies) {
            const bytes = cpEncoder.encode(value, 'ascii');
    ...
    public qrcode(
        ...
        if (this._embedded) {
            throw new Error(
                'QR codes are not supported in table cells or boxes',
            );
        }
        ...
        const bytes = cpEncoder.encode(value, 'iso88591');
     ...
     */
    public cut(value: 'full' | 'partial' = 'full') {
        if (this._embedded) {
            throw new Error('Cut is not supported in table cells or boxes');
        }

        this._queue([0x1d, 0x56, value === 'partial' ? 0x01 : 0x00]);
     ...
     * Pulse
     *
     * @param device  0 or 1 for on which pin the device is connected, default of 0
     * @param on  Time the pulse is on in milliseconds, default of 100
     * @param off  Time the pulse is off in milliseconds, default of 500
     */
    public pulse(device: number, on: number, off: number) {
        if (this._embedded) {
     ...
     */
    public raw(data: (number | Uint8Array)[]) {
        this._queue(data);
