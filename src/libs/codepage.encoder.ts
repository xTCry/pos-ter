...
export type KeysDefinitions = keyof typeof definitions;
...
export class CodepageEncoder {
     ...
     *
     */
    public static getEncodings() {
        return Object.keys(definitions) as KeysDefinitions[];
    }
     ...
     */
    public static getTestStrings(codepage: KeysDefinitions | string) {
        if (
     ...
     *
     */
    public static supports(codepage: KeysDefinitions | string) {
     ...
     *
     */
    public static encode(input: string, codepage: KeysDefinitions | string): Uint8Array {
     ...
     *
     */
    public static autoEncode(
        input: string,
        candidates: Partial<KeysDefinitions>[],
    ) {
        const fragments: {
            codepage: KeysDefinitions | string;
            bytes: Uint8Array | number[];
        }[] = [];
        let fragment = -1;
        let current: KeysDefinitions;

        for (let c = 0; c < input.length; c++) {
            const codepoint = input.codePointAt(c);

            let available: KeysDefinitions;
            let char = 0;
            ...
            if (!available) {
                for (let i = 0; i < candidates.length; i++) {
                    const position = definitions[candidates[i]].chars.indexOf(
                        input[c],
                    );
            ...
            if (current != available) {
                if (current) {
                    fragments[fragment].bytes = new Uint8Array(
                        fragments[fragment].bytes,
                    );
                }
            ...
            (fragments[fragment].bytes as number[]).push(char);
        }
